...
```sh
make docker-build docker-push IMG=<some-registry>/autodelpoy-scaler:tag
```
...
```sh
make build-installer IMG=<some-registry>/autodelpoy-scaler:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/autodelpoy-scaler/<tag or branch>/dist/install.yaml
```
